...
CREATE OR REPLACE VIEW bd.vue_jeu AS (
    SELECT J.numero_jeu, J.nom, J.editeur, J.date_de_parution, J.type_de_jeu, J.duree,
        CASE
         ...
         END as theme 
         FROM bd.jeu as J  
         LEFT OUTER JOIN bd.utilsation_mecanique as UM on J.numero_jeu=UM.numero_jeu  
         LEFT OUTER JOIN bd.mecanique as M on UM.numero_mecanique=M.numero_mecanique  
         LEFT OUTER JOIN bd.utilsation_theme as UT on J.numero_jeu=UT.numero_jeu  
         LEFT OUTER JOIN bd.theme as T on T.numero_theme = UT.numero_theme
);
...
CREATE OR REPLACE VIEW bd.vue_config AS (
    SELECT C.numero_configuration, C.nb_joueurs, J.numero_jeu, J.nom as nom_jeu, E.numero_extension, E.nom as nom_extension FROM bd.configuration as C
